...
public class Solution {
    ...
    static void addUser(User user) {
        ...
        int exceptionSetName = user.setName(name);
        if (exceptionSetName == -1)
            System.out.println(CANNOT_BE_NULL);
        else if (exceptionSetName == -2)
            System.out.println(CANNOT_BE_EMPTY);
        else if (exceptionSetName == -3)
            System.out.println(CANNOT_CONTAIN_DIGIT);
        else if (exceptionSetName != 0)
            System.out.println(UNKNOWN_ERROR);

        int exceptionSetAge = user.setAge(age);
        if (exceptionSetAge == -1)
            System.out.println(CANNOT_BE_NEGATIVE);
        else if (exceptionSetAge == -2)
            System.out.println(CANNOT_BE_TOO_BIG);
        else if (exceptionSetAge != 0)
            System.out.println(UNKNOWN_ERROR);
